...
from norm_josa import normalize_josa
...
def build_dataset_pair(binding, template):
    ...
    for variable in binding:
        ...
        if placeholder in english and label is not None:
            ...
            english = normalize_josa(english, label)
...
def get_results_of_generator_query( cache, template ):
    ...
    for attempt, prepare_query in enumerate([first_attempt, second_attempt], start=1):
...
LABEL_REPLACEMENT = "  (str(?lab{variable}) as ?l{variable}) where {{ ?{variable} rdfs:label ?lab{variable} . FILTER(lang(?lab{variable}) = 'ko') . "
#CLASS_REPLACEMENT = " where {{ ?{variable} a {ontology_class} . "
CLASS_REPLACEMENT = " where {{ ?{variable} wdt:P31 {ontology_class} . "
#CLASSES_REPLACEMENT = " where {{ ?{variable} a ?t . VALUES (?t) {{ {classes} }} . "
CLASSES_REPLACEMENT = " where {{ ?{variable} wdt:P31 ?t . VALUES (?t) {{ {classes} }} . "
